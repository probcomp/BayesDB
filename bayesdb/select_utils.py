...
def evaluate_where_on_row(row_idx, row, where_conditions, M_c, M_c_full, X_L_list, X_D_list, T, T_full, engine, tablename, numsamples, impute_confidence, T_full_imputed, T_full_imputed_confidences, cctypes_full, resolution):
  """
  ...
  for (func, f_args, op, val) in where_conditions:
    if func == functions._column and f_args[1] != None and numpy.isnan(T[row_idx][f_args[0]]):
      ...
      if func != functions._column_ignore:
        ...
        where_value = func(f_args, row_idx, row, M_c_full, T_full, engine, T_full_imputed, T_full_imputed_confidences, cctypes_full)
