...
The output is a list sampled values $x$ with the same ordering of variables as Q,
$$
x \sim Pr[Q|X_L,Y].
$$
...
\Input{$M_c, X_L, X_D, Y, q_r, Q_c$.}
\Output{A list of sampled values $x$.}
\BlankLine
\tcp{Separate the conditions, $Y$, by whether they apply to the query row, $q_r$}
$Y^q \leftarrow Y s.t. \; Y_{*,1}=q_r$ \;
$Y^o \leftarrow Y s.t. \; Y_{*,1}!=q_r$ \;
\ForEach{$view_i \in views $}{
  \eIf{$q_r$ denotes an observed row}{
    \tcp{The category is determinstically set}
    $c_{view_i} \leftarrow get\_category(X_D, view_i, q_r)$ \;
  }{
    \tcp{sample a category, allowing the possibility of choosing a new category}
    $Pr(c | view, Y^q, Y^o, X_L) \propto Pr(Y^q | y', view, Y^o, X_L)Pr(y'|view, X_L)$ \;
    $c_{view_i} \sim Pr(c | view_i, Y^q, Y^o, X_L)$ \;
  }
}
\ForEach{$q_i \in Q_c$}{
  $view_i \leftarrow get\_view(X_L, q_i)$ \;
  $cm \leftarrow get\_component\_model(X_L, view_i, c_{view_i}, q_i)$ \;
  \tcp{Determine constraints that modify this component model}
  $Y^c = Y s.t. \; Y_{*,1} = q_i \; and \; get\_category(X_D, view_i, Y_{*,2}) = c_{view_i}$ \;
