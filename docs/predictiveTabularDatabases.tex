...
\item \texttt{initialize($M_c, M_r, T, i$)} $\rightarrow \{ M_c, M_r, X_L, X_D \}$. \\
...
\item \texttt{simple\_predictive\_sample($M_c, X_L,X_D,Y,q_r, q_C$)} $\rightarrow x$. \\
$q_C$ is a list of columns on a single row $q_r$ to be simultaneously sampled given the conditions $Y$.  The output is a list sampled values $x$ with the same ordering as $q_C$,
$$
x \sim Pr[T_{q_r, q_C}|X_L,Y].
$$

% SimplePredictiveProbability
\item \texttt{simple\_predictive\_probability($M_c, X_L,X_D,Y_{cond},Y_{query},n$)} $\rightarrow p$. \\
$Y_{cond}$ is a list of conditioning conditions, while $Y_{query}$ is a list of queried conditions to be simultaneously satisfied given the conditioning conditions are true.  $n$ is the number of samples used to approximate the predictive probability.  The output, $p$, is the marginal predictive probability of $Y_{query}$ given $Y_{cond}$, assuming the model in $X_L$,
$$
p = Pr[Y_{query} | X_L,Y_{cond}].
$$
...
Extract the queried row, column and hypothesized value:\\
$r,c,h \leftarrow Q_i$

Find all elements of $Y$ that apply to the same column:
\[Y' \leftarrow Y s.t. \; Y_{*,2}=c\]
...
p_i =  p_i + PP(Q_i | \vec{\tau^{f}_{category}}) 
\] 
}
