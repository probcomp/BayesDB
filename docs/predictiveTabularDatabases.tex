...
\item \texttt{simple\_predictive\_probability($M_c, X_L,X_D,Y,Q,n$)} $\rightarrow p$. $Y$ is a list of conditioning conditions (indices paired with values), while  
$Q$ is a list of queried conditions (a indices paired with values). $n$ is the number of samples used to approximate the predictive probability.
...
% ConditionalEntropy
\item \texttt{conditional\_entropy($M_c, X_L,X_D,d_{given},d_{target}, [n, maxTime]$)} $\rightarrow e$. $d_{given}$ and $d_{target}$ are lists of column indices, $n$ is the number of predictive samples to base the estimate on, subject to $maxTime$ which is the maximum amount of time allowed for computation. The output is an estimate of the conditional entropy of $d_{target}$ given $d_{given}$,
...
\Input{$M_c, X_L,X_D,d_{given},d_{target},n,maxTime$}
