...
Similarly, we can list the variables that share the most dependence with spending, again focusing on Medicare spending on ambulances.
...
<!--
To dig further into hospitals like McAllen, we can sort by similarity with respect to quality,

    SELECT qual_score, mdcr_spnd_amblnc FROM dha ORDER BY SIMILARITY TO name="McAllen TX" WITH RESPECT TO qual_score LIMIT 10

|        name       | qual_score | mdcr_spnd_amblnc |
|-------------------|------------|------------------|
|     McAllen TX    |    69.5    |      3037.0      |
|     Monroe LA     |    77.1    |      1258.0      |
|      Miami FL     |    87.5    |      914.0       |
|    Harlingen TX   |    85.0    |      1557.0      |
|     El Paso TX    |    77.3    |      661.0       |
|     Chicago IL    |    80.8    |      921.0       |
|  New Brunswick NJ |    95.0    |      704.0       |
|     Abilene TX    |    79.2    |      546.0       |
|      Akron OH     |    89.1    |      980.0       |
| Alameda County CA |    87.1    |      1096.0      |

Monroe, El Paso, New Brunswick, and Abilene all also appeared on the list of suprising entries, suggesting that an entry that is similar to a surprising entry is likely to also be surprising. 

Another method of following up on the surprisingness result is to simply order entries from lowest quality to highest.

    SELECT qual_score, mdcr_spnd_amblnc FROM dha ORDER BY qual_score ASC LIMIT 10
...
|     Monroe LA     |    77.1    |      1258.0      |
|     El Paso TX    |    77.3    |      661.0       |
|     Lubbock TX    |    78.1    |      606.0       |
|      Macon GA     |    78.6    |      555.0       |
|   Montgomery AL   |    79.0    |      787.0       |
|     Abilene TX    |    79.2    |      546.0       |
| Corpus Christi TX |    79.3    |      1831.0      |
|   Gainesville FL  |    80.1    |      687.0       |
|     Chicago IL    |    80.8    |      921.0       |

McAllen, Monroe, El Paso, Abilene, and Chicago all have low quality scores and are suprising. 

We might also want to know why the other entries appeared on our suprising list. One place to start is to compare those entries with the list of high quality hospitals.

    SELECT qual_score, mdcr_spnd_amblnc FROM dha ORDER BY qual_score DESC LIMIT 10

|       name       | qual_score | mdcr_spnd_amblnc |
|------------------|------------|------------------|
|    Elyria OH     |    95.6    |      843.0       |
|     Minot ND     |    95.6    |      249.0       |
| New Brunswick NJ |    95.0    |      704.0       |
|  Mason City IA   |    94.6    |      344.0       |
| Grand Rapids MI  |    94.4    |      734.0       |
|   La Crosse WI   |    94.3    |      410.0       |
|    Muncie IN     |    93.9    |      553.0       |
|   Rochester MN   |    93.8    |      461.0       |
|  Morristown NJ   |    93.4    |      473.0       |
|   Muskegon MI    |    93.4    |      625.0       |

Elyria, Minot, and Muncie are hospitals with high quality scores and are surprising. These results agree with the intuition that there is a greater possibility of surprise when one is far way from the average. However, the results do not explain why these particular hospitals were suprising. Looking at the spending can give us some hints. Minot is surprising because it obtains a high quality score while spending very little relatively. Similarly, Muncie's spending is on the low side. Returning to the list of hospitals with low quality scores, we see that McAllen spends a very large amount relatively and Monroe is also on the high side for spending. 
