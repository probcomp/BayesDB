...
We can see that there are large blocks of variables that are probably dependent, as indicated by the dark blue, as well as variables that are probably independent of one another, as indicated by the white. The key variables for this analysis are the number of books in the home, `p1chlboo`, and outcomes. There are multiple outcome measures in the dataset, and we will focus on kindergarten performance on math and reading: `c1r4mtsc` and `c1r4rtsc`. To assess the probability of dependence, we can create a column list with only these variables, and print out BayesDB’s estimate.
    ...
    CREATE COLUMN LIST p1chlboo, C1R4MTSC, C1R4RTSC FROM eccls AS books_outcomes; 
    ESTIMATE PAIRWISE DEPENDENCE PROBABILITY FROM ecls FOR books_outcomes;

FIXME

BayesDB says there is a moderate probability of dependence between reading and math scores in kindergarten and the number of books in the home. 

Where does the number of books in the home lie relative to other possible predictors? To investigate this question, we can sort the variables based on the estimated probability of dependence with the target variable. Here we narrow our focus to just maths scores, as that is the more interesting potential association. (It is pretty clear why having books may be related to reading scores, less so for math scores.)
