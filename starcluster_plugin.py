...
class CrosscatSetup(ClusterSetup):
     ...
     def run(self, nodes, master, user, user_shell, volumes):
          ...
          checkout_tuple = (S.path.remote_code_dir, S.git_branch)
          #
          for node in nodes:
          ...
          for node in nodes:
               ...
               node.ssh.execute('apt-get install -y libpng12-dev')
               node.ssh.execute('bash %' % S.path.install_boost_script)
               node.ssh.execute('bash %' % S.path.virtualenv_setup_script)
